...
return {
  -- 1️⃣ Mason: менеджер LSP-серверов и форматтеров
  {
    ...
    opts = {
      ensure_installed = {
        ...
        "html-lsp", "css-lsp", "json-lsp", "bash-language-server",
        -- 🧹 Форматтеры
        "gofumpt", "goimports", "stylua", "prettier", "clang-format",
      },
    },
  },
  ...
  -- 3️⃣ LSP конфигурация через НОВЫЙ API (Neovim 0.11+)
  {
    ...
    config = function()
      ...
      -- 🐹 Go
      vim.lsp.config("gopls", vim.tbl_extend("force", common_opts, {
        settings = {
          gopls = {
            gofumpt = true,
            staticcheck = true,
      ...
      -- 🦀 Rust
      vim.lsp.config("rust_analyzer", vim.tbl_extend("force", common_opts, {
        settings = {
          ["rust-analyzer"] = {
      ...
      -- 🐘 C / C++
      vim.lsp.config("clangd", vim.tbl_extend("force", common_opts, {
        cmd = { "clangd", "--background-index", "--clang-tidy" },
      }))

      -- 🌙 Lua
      vim.lsp.config("lua_ls", vim.tbl_extend("force", common_opts, {
        settings = {
          Lua = {
      ...
      vim.lsp.config("eslint", vim.tbl_extend("force", common_opts, {
